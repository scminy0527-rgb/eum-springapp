...
public class PrivateChatApi {
    ...
    @Operation(summary = "채팅 이미지 업로드", description = "이미지를 서버에 저장하고 URL을 반환합니다. 반환된 URL을 WebSocket으로 전송하세요. (로그인 필요)")
    ...
    public ResponseEntity<ApiResponseDTO> uploadChatImage(
            @PathVariable Long chatRoomId,
            @RequestPart("file") MultipartFile file
    ) throws IOException {
        String url = chatService.uploadChatImage(file);
        return ResponseEntity.ok(ApiResponseDTO.of(true, "이미지 업로드 성공", url));
    }
}
